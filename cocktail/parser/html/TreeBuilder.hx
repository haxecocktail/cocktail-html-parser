...
class TreeBuilder
{
		...
		switch (m)
		{
			case INITIAL:
				switch (t)
				{
					...
					case DOCTYPE(name, publicId, systemId, forceQuirks):
						...
						// doc.doctype = dt; <= that shouldn't be necessary as it should be implemented in Document when using 
						//						appendChild, insertBefore, replaceChild, (...) on it.
			...
			case BEFORE_HTML:
				switch (t)
				{
					...
					#else
						var e = doc.documentElement;
						overrideHTMLElement(e, START_TAG("html", false, new Map()));
					#end
						//TODO If the Document is being loaded as part of navigation of a browsing context, 
			...
			case BEFORE_HEAD:
				switch (t)
				{
					...
					#else
						hp = overrideHTMLElement(doc.head, t);
					#end
						im = IN_HEAD;
					case END_TAG(tg, _, _) if (tg!="head" && tg!="body" && tg!="html" && tg!="br"):
						//TODO parse error
			...
			case IN_HEAD:
				switch (t)
				{
					...
					case START_TAG("meta", sc, _):
						...
						//ASCII-compatible character encoding or a UTF-16 encoding, and the confidence is currently tentative, then 
			...
			case AFTER_HEAD:
				switch (t)
				{
					...
					case START_TAG("body", _, _):
						...
						insertHTMLElement(t);
					#else
						overrideHTMLElement(doc.body, t);
					#end
						framesetOK = false;
						im = IN_BODY;
					case START_TAG("frameset", _, _):
						insertHTMLElement(t);
						im = IN_FRAMESET;
					case START_TAG("base", _, _)|START_TAG("basefont", _, _)|START_TAG("bgsound", _, _)|START_TAG("link", _, _)|
						 START_TAG("meta", _, _)|START_TAG("noframes", _, _)|START_TAG("script", _, _)|START_TAG("style", _, _)|
						 START_TAG("title", _, _):
						//TODO parse error
			...
			case IN_BODY:
				switch (t)
				{
					...
					case START_TAG("base", _, _) | START_TAG("basefont", _, _) | START_TAG("bgsound", _, _) | 
						 START_TAG("command", _, _) | START_TAG("link", _, _) | START_TAG("meta", _, _) | 
						 START_TAG("noframes", _, _) | START_TAG("script", _, _) | START_TAG("style", _, _) | 
						 START_TAG("title", _, _):
							doProcessToken(t, IN_HEAD);
	...
	#if !js
		doc.readyState = "interactive"; // FIXME where should this be managed ?
	#end
	...
	#if !js
		doc.readyState = "complete"; // idem as before
	#end
	}
	/**
	...
	function overrideHTMLElement(e : Element, t : Token) : Element
	{
		e = overrideElement(e, t);
		stack.push(e);
		return e;
	}
	function overrideElement(e : Element, t : Token) : Element
	{
		switch (t)
		{
			case START_TAG(_, _, attrs): // TODO check tagname ?
				for (i in 0...e.attributes.length)
				{
					e.removeAttribute(Std.instance(e.attributes.item(i), Attr).name);
				}
				for (att in attrs.keys())
				{
					e.setAttribute(att, attrs.get(att));
				...
				return e;
			case _:
				throw "ERROR cannot override element for that token";
		}
	}
	/**
	 * 
