...
/**
 * There is no actual HTMLSerializer interface in the specs. There is however a XMLSerializer. It thus seems 
	...
	static public function serialize( node : Node ) : String
	{
		...
		for ( currentNode in node.childNodes )
		{
			if ( currentNode.nodeType == Node.ELEMENT_NODE )
			{
				...
				{
					s.addChar( 0x20 );
					s.add( serializeAttributeName( attr.nodeName ) );
					s.addChar( 0x3D );
					s.addChar( 0x22 );
					s.add( escape( attr.nodeValue ) );
					s.addChar( 0x22 );
				}
				s.addChar(0x3E);
